...
**差分取得の前に、main ブランチに未コミットの変更がないか確認する。**

```bash
git -C ~/dotfiles-mac status --short
```

- 未コミットの変更（M, A, D など）がある場合は、**作業を止めてユーザーに報告し、コミットするかどうかを確認する。**
- 未追跡ファイル（`??`）のみの場合は無視して続行する。
- `git diff --name-only linux main` はブランチ間のコミット済み差分のみを見るため、未コミット変更は含まれない。移植前にコミットしないと意図した変更が linux に届かない。
