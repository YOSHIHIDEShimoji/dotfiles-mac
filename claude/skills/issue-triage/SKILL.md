...
**裏取りしてから判断する**: Issue 本文の主張を鵜呑みにしない。特に `P0-critical` / `P1-high` / `risk-high` を付ける前に、該当コードを読む・再現コマンドを実行するなど、**その深刻度を言い切れる根拠**を最低1つ確認する。裏が取れなければ憶測で盛らず、確認できた事実に基づくラベルに格下げする。
