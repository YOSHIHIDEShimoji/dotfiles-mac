...
| `install/Aptfile`・`install/bootstrap-linux.sh` | 「Linux/WSL 対応」 |
...
- OS 固有の機能は「Linux/WSL 対応」セクションと `docs/platform-notes.md` の記述と矛盾させない（どちらが Mac 専用/Linux 専用かは platform-notes.md が正）
- 関数テーブルの説明は簡潔に（1行以内）
