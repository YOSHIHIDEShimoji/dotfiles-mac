...
# ─── Aptfile 読み込み ────────────────────────────────────
# [wsl], [linux] セクションから対応プラットフォームのパッケージを取得する
get_packages() {
    local target="$1"
    local section="" line
...
# ─── 2. zsh の確認（chsh 前に確保）─────────────────────────
# zsh は [wsl] に含まれるが、chsh より前に確実に入れる
...
if [ ! -d "$PYENV_ROOT" ]; then
    ...
    export PATH="$PYENV_ROOT/bin:$PATH"  # インストール直後に PATH を通す
else
    info "pyenv を更新します..."
    git -C "$PYENV_ROOT" pull --ff-only
    git -C "$PYENV_ROOT/plugins/python-build" pull --ff-only 2>/dev/null || true
fi

if command -v pyenv &>/dev/null; then
    info "pyenv $(pyenv --version) を確認しました。"
else
...
echo "════════════════════════════════════════════════"
echo " 次のステップ:"
echo "   1. ターミナルを閉じて新しく開き直す（source ~/.zshrc だけでは不十分）"
echo "      → pyenv / starship 等が正しく読み込まれます"
echo ""
echo " オプション（必要に応じて）:"
