...
| コマンド | 概要 |
|---------|------|
| `transcribe` | 音声・動画ファイルを文字起こし（m4a/mp4等対応）。whisper.cpp + CoreML で M4 Neural Engine を活用。1時間の音声を約3分で処理 |
| `excel2csv` | Excel ファイル（.xlsx）を CSV に一括変換 |
| `launchd_list` | `~/dotfiles-mac/LaunchAgents/` 配下のジョブを一覧表示（スケジュール・次回実行時刻・ステータス付き） |
